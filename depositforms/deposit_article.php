...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<?php if($_SERVER['REQUEST_METHOD'] == 'POST'): ?>
        ...
        <form method="post" accept-charset="utf-8" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
            <p>
                <label id="titlelabel" class="floatable" for="title">Title: </label> <input id="title" name="title" size="55" value="<?php if(($_SERVER['REQUEST_METHOD'] == 'POST' && $_SESSION['form_valid'] == true) || $_SERVER['REQUEST_METHOD'] == 'GET') { echo ''; } else { echo $_POST['title']; } ?>Title for today"/>
            </p>

	    <div id="authorcontainer">
		<p class="firstauthor">
                    <label id="familylabel_1" class="floatable" for="authorfamily_1">Author Family Name: </label> <input id="authorfamily_1" name="authorfamily[]" size="55" value="<?php if(($_SERVER['REQUEST_METHOD'] == 'POST' && $_SESSION['form_valid'] == true) || $_SERVER['REQUEST_METHOD'] == 'GET') { echo ''; } else { echo $_POST['authorfamily'][0]; } ?>V2"/>
                </p>
                <p class="firstauthor">
                    <label id="givenlabel_1" class="floatable" for="authorgiven_1">Author Given Name/Initials: </label> <input id="authorgiven_1" name="authorgiven[]" size="55" value="<?php if(($_SERVER['REQUEST_METHOD'] == 'POST' && $_SESSION['form_valid'] == true) || $_SERVER['REQUEST_METHOD'] == 'GET') { echo ''; } else { echo $_POST['authorgiven'][0]; } ?>SWORDAPI"/>
		</p>
            <div id="journalcontainer">
		<p>
                    <label id="journallabel" class="floatable" for="journal_title">Journal: </label> <input id="journal_title" name="journal_title" size="55" value="<?php if(($_SERVER['REQUEST_METHOD'] == 'POST' && $_SESSION['form_valid'] == true) || $_SERVER['REQUEST_METHOD'] == 'GET') { echo ''; } else { echo $_POST['journal_title']; } ?>Journal of Irreproducible Results"/>
                </p>
		<p>
		...
		<p>
                    <label id="namelabel" class="floatable" for="name">Your name: </label> <input id="name" name="name" size="55"/ value="<?php if(isset($_POST['email'])) { echo $_POST['name']; } else { echo ''; } ?>Roger Depositor">
                </p>
		<p>
                    <label id="emaillabel" class="floatable" for="email">Your email: </label> <input id="email" name="email" size="55" value="<?php if(isset($_POST['email'])) { echo $_POST['email']; } else { echo ''; } ?>tommy@library.caltech.edu"/>
                </p>
		<p>
	...
	#print_r($_FILES);
		...
		if($_SESSION['form_valid'] == true)
		{
		    ...
		    $wrapper = new EPrintsWrapper('http://authorstest.library.caltech.edu/sword-app/servicedocument', 'swordtester', 'sWTSTER');
		    for($i=0; $i<count($_POST['authorgiven']); $i++)
		    {
			$wrapper->addCreator(trim($_POST['authorfamily'][$i]), trim($_POST['authorgiven'][$i]), null);
		    }
		    
		    $wrapper->note = trim($_POST['note']);
		    $wrapper->title = trim($_POST['title']);
		    ...
		    $wrapper->addEPrintMetadata($wrapper->title, "article");
			...
			for($i=0; $i<$filecount; $i++)
			{
				...
				if($success)
				{
				    // not in use for addFile2$wrapper->addFile($_FILES['fileupload']['tmp_name'][$i], $new_id, $_FILES['fileupload']['type'][$i], $_FILES['fileupload']['name'][$i]);

				    $wrapper->addFile2($_FILES['fileupload']['tmp_name'][$i], $_FILES['fileupload']['type'][$i], $_FILES['fileupload']['name'][$i]);
				}
			    }
			}
		    }
		    ...
		    $new_id = $wrapper->commitNewEPrint();
		    error_log("new_id = $new_id");

		    if($new_id == EPrintsWrapper::ERROR_VALUE)
		    {
			error_log($wrapper->getErrorMessage());
		    }
